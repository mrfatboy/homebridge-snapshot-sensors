...
describe('resolveSensors()', () => {
  ...
  it('multiple categories → one OR-sensor, auto-named', () => {
    ...
    expect(s).toEqual({
      name: 'Garden Animals & People Sensor',
      categories: ['animals', 'people'],
      thresholds: { animals: THRESHOLD, people: THRESHOLD },
      unidentifiedMotionActivity: true,
    });
  });
  ...
  it('applies category-specific thresholds to the sensor', () => {
    const [s] = resolveSensors(
      'Garden',
      [{ categories: ['animals', 'vehicles'], thresholds: { animals: 0.3, vehicles: 0.7 } }],
      () => {},
    );
  ...
  it('explicit name is used verbatim (no stream prefix)', () => {
    const [s] = resolveSensors(
      'Garden',
      [{ name: 'Front Door', categories: ['people'] }],
      () => {},
    );
    expect(s).toEqual({
      name: 'Front Door',
      categories: ['people'],
      thresholds: { people: THRESHOLD },
      unidentifiedMotionActivity: true,
    });
  });
  ...
  it('clamps an out-of-range category threshold back to the default', () => {
    ...
    const [s] = resolveSensors(
      'Garden',
      [{ categories: ['animals'], thresholds: { animals: 5 } }],
      warn,
    );
