...
describe('NotificationService', () => {
  it('sends Pushover notifications and requires provider-level success', async () => {
    vi.stubGlobal(
      'fetch',
      fetchMock.mockResolvedValue(response(200, JSON.stringify({ status: 1 }))),
    );
    ...
    await expect(
      NotificationService.send({ notification, title: 'Title', message: 'Message' }),
    ).resolves.toEqual({ sent: true, provider: 'Pushover', status: 200 });
  ...
  it('rejects a Pushover API failure even when HTTP succeeds', async () => {
    vi.stubGlobal(
      ...
      fetchMock.mockResolvedValue(
        response(200, JSON.stringify({ status: 0, errors: ['invalid token'] })),
      ),
    );
    const notification = {
      provider: 'pushover' as const,
      pushover: { token: 'bad', user: 'user' },
    };
    await expect(
      NotificationService.send({ notification, title: 'Title', message: 'Message' }),
    ).rejects.toThrow('Pushover returned invalid token');
  });
  ...
  it('rejects multiple Pushbullet targets', async () => {
    ...
    await expect(
      NotificationService.send({ notification, title: 'Title', message: 'Message' }),
    ).rejects.toThrow('Specify only one Pushbullet target');
  ...
  it('sends Pushbullet notifications to a channel when configured', async () => {
    ...
    const notification = {
      provider: 'pushbullet' as const,
      pushbullet: { apiKey: 'key', channelTag: 'alerts' },
    };
    await expect(
      NotificationService.send({ notification, title: 'Title', message: 'Message' }),
    ).resolves.toEqual({ sent: true, provider: 'Pushbullet', status: 200 });
  ...
  it('uses the ntfy default server and preserves notification options', async () => {
    ...
    await expect(
      NotificationService.send({ notification, title: 'Title', message: 'Message' }),
    ).resolves.toEqual({ sent: true, provider: 'ntfy', status: 200 });
    const [url, options] = fetchMock.mock.calls[0];
  ...
  it('validates Pushcut HTTPS URLs', async () => {
    vi.stubGlobal('fetch', fetchMock);
    const notification = {
      provider: 'pushcut' as const,
      pushcut: { pushcutUrl: 'http://example.test/hook' },
    };
    await expect(
      NotificationService.send({ notification, title: 'Title', message: 'Message' }),
    ).rejects.toThrow('Pushcut Webhook URL must use HTTPS');
    expect(fetchMock).not.toHaveBeenCalled();
  });
  ...
  it('sends Pushcut JSON using the shared message', async () => {
    ...
    const notification = {
      provider: 'pushcut' as const,
      pushcut: { pushcutUrl: 'https://example.test/hook' },
    };
    await expect(
      NotificationService.send({ notification, title: 'Title', message: 'Message' }),
    ).resolves.toEqual({ sent: true, provider: 'Pushcut', status: 200 });
    const [, options] = fetchMock.mock.calls[0];
  ...
  it('sends Push Safer sound and optional fields from the same provider implementation', async () => {
    ...
    await expect(
      NotificationService.send({ notification, title: 'Title', message: 'Message', sound: '7' }),
    ).resolves.toEqual({ sent: true, provider: 'Push Safer', status: 200 });
    const [, options] = fetchMock.mock.calls[0];
