...
export function takeLatestFrame(
  ...
  return {
    latest: Buffer.from(buf.subarray(lastStart, lastStart + frameBytes)),
    rest: Buffer.from(buf.subarray(whole * frameBytes)),
  };
}

const FFMPEG_ARGS = (url: string): string[] => [
  '-loglevel', 'error', '-i', url, '-an',
...
export class FfmpegPump {
  ...
  private evaluateHealth(): void {
    const now = Date.now();
    const health: StreamHealth = this.everFramed && now - this.lastFrameAt <= FFMPEG_TIMEOUT_FRAME_MS
      ? 'online'
      : !this.everFramed && now - this.startedAt <= FFMPEG_TIMEOUT_FRAME_MS ? 'connecting' : 'down';
