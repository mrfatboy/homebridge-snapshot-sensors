...
function normalizeDetection(value: unknown): Detection | null {
  ...
  return {
    x1: Number(item.x1 ?? item.left ?? 0),
    y1: Number(item.y1 ?? item.top ?? 0),
    x2: Number(item.x2 ?? item.right ?? 0),
    y2: Number(item.y2 ?? item.bottom ?? 0),
    score,
    classId: Number.isFinite(classId) ? classId : -1,
    className,
  };
}
...
export async function runYolo(
  ...
  const endpoint = storeSnapshots === 'annotated' ? '/detect-annotated' : '/detect';
  const form = new FormData();
  form.append('file', new Blob([image], { type: 'image/jpeg' }), 'snapshot.jpg');
  // The YOLO service uses this value to know whether the caller intends to keep
  // the original image, an annotated image, or no saved image at all.
  ...
  try {
    ...
    if (contentType.startsWith('image/')) {
      return { detections: [], annotatedImage: Buffer.from(await response.arrayBuffer()), contentType };
    }

    const body = (await response.json()) as Record<string, unknown>;
    const annotatedImage = decodeImage(
      body.annotated_image ?? body.annotatedImage ?? body.image ?? body.image_base64,
    );
    ...
    if (error instanceof DOMException && error.name === 'AbortError') {
      throw new Error(`YOLO request timed out after ${YOLO_TIMEOUT_MS}ms`);
    }
    throw error;
  } finally {
