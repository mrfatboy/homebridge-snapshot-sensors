(() => {
  ...
  function sensorHtml() {
    return `<div class="sensor-settings"><div class="mb-2 sensor-heading"><strong><span class="sensor-emoji">📡</span> Sensor</strong></div><div class="form-group mb-3"><label>Categories and Detection Thresholds - Range 0-1. Smaller is more sensitive</label><div class="form-check d-flex align-items-center mb-2"><input class="form-check-input category" type="checkbox" value="animals" checked><label class="form-check-label ms-2 flex-grow-1">🐕 Animal</label><input class="form-control category-threshold" data-category="animals" type="number" min="0" max="1" step="0.01" value="0.25" style="width:90px"></div><div class="form-check d-flex align-items-center mb-2"><input class="form-check-input category" type="checkbox" value="people" checked><label class="form-check-label ms-2 flex-grow-1">🚶‍♂️ Person</label><input class="form-control category-threshold" data-category="people" type="number" min="0" max="1" step="0.01" value="0.25" style="width:90px"></div><div class="form-check d-flex align-items-center"><input class="form-check-input category" type="checkbox" value="vehicles" checked><label class="form-check-label ms-2 flex-grow-1">🚗 Vehicle</label><input class="form-control category-threshold" data-category="vehicles" type="number" min="0" max="1" step="0.01" value="0.25" style="width:90px"></div></div></div>`;
  }
  ...
  function updateNotificationVisibility(snapshot) {
    ...
    const pushsafer = snapshot.querySelector('.pushsafer-controls');
    ...
    pushsafer.style.display = notifications.value === 'pushsafer' ? '' : 'none';
  }
  ...
  snapshots.addEventListener('click', async event => {
    ...
    if (event.target.closest('.test-notification')) {
      ...
      } else if (provider === 'pushsafer') {
        const privateKey = snapshot.querySelector('.pushsafer-private-key').value.trim();
        const pushsaferDevice = snapshot.querySelector('.pushsafer-device').value.trim();
        const title = snapshot.querySelector('.pushsafer-title').value.trim();
        const icon = Number(snapshot.querySelector('.pushsafer-icon').value || 1);
        const vibration = Number(snapshot.querySelector('.pushsafer-vibration').value || 1);
        const iconColor = snapshot.querySelector('.pushsafer-icon-color').value.trim();
        const sound = snapshot.querySelector('.pushsafer-sound').value.trim();
        const url = snapshot.querySelector('.pushsafer-url').value.trim();
        const urlTitle = snapshot.querySelector('.pushsafer-url-title').value.trim();
        const priority = Number(snapshot.querySelector('.pushsafer-priority').value || 0);
        if (!privateKey) { homebridge.toast.error('Please enter the Push Safer Private Key.', 'Invalid Push Safer Settings'); return; }
        if (!title) { homebridge.toast.error('Please enter the Push Safer Title.', 'Invalid Push Safer Settings'); return; }
        requestBody = { provider, privateKey, pushsaferDevice, title, icon, vibration, iconColor, sound, url, urlTitle, priority };
      } else {
      ...
      try {
        ...
        homebridge.toast.success('Test notification sent successfully.', provider === 'pushbullet' ? 'Pushbullet' : provider === 'ntfy' ? 'ntfy' : provider === 'pushsafer' ? 'Push Safer' : 'Pushover');
      } catch (error) {
