(() => {
  ...
  function updateNotificationVisibility(snapshot) {
    ...
    const pushcut = snapshot.querySelector('.pushcut-controls');
    pushover.style.display = notifications.value === 'pushover' ? '' : 'none';
    pushcut.style.display = notifications.value === 'pushcut' ? '' : 'none';
  }
  ...
  snapshots.addEventListener('click', async event => {
    ...
    if (event.target.closest('.test-notification')) {
      ...
      if (provider === 'pushcut') {
        const apiKey = snapshot.querySelector('.pushcut-api-key').value.trim();
        const notificationName = snapshot.querySelector('.pushcut-notification-name').value.trim();
        const device = snapshot.querySelector('.pushcut-device').value.trim();
        const sound = snapshot.querySelector('.pushcut-sound').value.trim();
        if (!apiKey) { homebridge.toast.error('Please enter the Pushcut API Key.', 'Invalid Pushcut Settings'); return; }
        if (!notificationName) { homebridge.toast.error('Please enter the Pushcut Notification Name.', 'Invalid Pushcut Settings'); return; }
        if (!sound) { homebridge.toast.error('Please enter the Pushcut Sound.', 'Invalid Pushcut Settings'); return; }
        requestBody = { provider, apiKey, notificationName, device, sound };
      } else {
      ...
      try {
        ...
        homebridge.toast.success('Test notification sent successfully.', provider === 'pushcut' ? 'Pushcut' : 'Pushover');
      } catch (error) {
    ...
    if (event.target.closest('.test-snapshot')) {
      ...
      button.textContent = '📸 Testing...';
      try {
        const result = await homebridge.request('/test-snapshot', { url, directory, prefix, storeSnapshots });
