  ...
  snapshots.addEventListener('click', async event => {
    ...
    if (event.target.closest('.test-notification')) {
      ...
      if (provider === 'pushbullet') {
        ...
        const body = snapshot.querySelector('.pushbullet-body').value.trim();
        ...
        if (!body) { homebridge.toast.error('Please enter the Pushbullet Message.', 'Invalid Pushbullet Settings'); return; }
        if (deviceIden && email) { homebridge.toast.error('Specify only one Pushbullet device target: Device Identifier or Email.', 'Invalid Pushbullet Settings'); return; }
        if (deviceIden && channelTag || email && channelTag) { homebridge.toast.error('Specify only one Pushbullet target: device/email or channel tag.', 'Invalid Pushbullet Settings'); return; }
        requestBody = { provider, apiKey, deviceIden, email, channelTag, title, body };
      } else {
      ...
      try {
        ...
        homebridge.toast.success('Test notification sent successfully.', provider === 'pushbullet' ? 'Pushbullet' : 'Pushover');
      } catch (error) {
