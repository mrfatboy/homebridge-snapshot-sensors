(() => {
  ...
  const addControls = (card) => {
    ...
    const pushsafer = card.querySelector('.pushsafer-controls');
    if (pushsafer) pushsafer.insertAdjacentHTML('afterend', webhookHtml());
    else notifications.insertAdjacentHTML('afterend', webhookHtml());
    update(card);
  };
  const update = (card) => {
    const enabled = card.querySelector('.webhook-enabled');
    const settings = card.querySelector('.webhook-settings');
    const url = card.querySelector('.webhook-url');
    const test = card.querySelector('.test-webhook');
    if (!enabled || !settings || !url || !test) return;
    settings.style.display = enabled.checked ? '' : 'none';
    test.disabled = !enabled.checked || !url.value.trim();
  };
  ...
  document.querySelector('#snapshots').addEventListener('change', (event) => {
    ...
    if (card && event.target.classList.contains('webhook-enabled')) update(card);
  });
